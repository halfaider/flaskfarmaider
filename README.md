...
- 시작 스크립트
    시작시 한번 실행되어 Flaskfarm 실행에 필요한 쉘커맨드를 실행
- Plexmate 파일 정리
...
- ### 최대 스캔 시간
    ...
    Plexmate의 스캔 항목 중에서 `SCANNING` 상태가 이 시간을 초과하여 지속될 경우 스캔 실패로 간주합니다. 해당 항목들은 Plexmate Ready 새로고침이 실행될 때 다시 `READY`로 변경됩니다. 계속해서 `SCANNING` 상태에 빠지는 항목은 수동으로 DB 삭제해 주세요.
...
- **새로고침 후 스캔**
    ...
    | 항목 | 값 | 예시 |
    | --- | --- | --- |
    | 설명 | 선택 | VOD 드라마를 새로고침 후 스캔 |
    | 로컬 경로 | **필수** | /mnt/gds/VOD/1.방송중/드라마 |
    | VFS 리모트 | **필수** (기본값: 설정 메뉴의 `리모트` 값) | gds: |
    | recursive | 선택 | Off |
    | 스캔 방식 | **필수** | 주기적 스캔 |
    | 주기적 스캔 작업 | 스캔 방식이 `주기적 스캔`일 경우 **필수** | 1. 최신 드라마: 최신드라마 주기적 스캔 |
    | 일정 방식 | 선택 | 시간 간격 |
    | 시간 간격 | 일정 방식에 따라... | 60 |
    | 시작시 일정 등록 | 선택 | On |
...
- **스캔**
    ...
    | 스캔 방식 | **필수** | 주기적 스캔
    | 주기적 스캔 작업 | 스캔 방식에 따라... | 2. 국내 드라마: 국내TV/드라마|
    | 일정 방식 | 선택 | 시간 간격 |
    | 시간 간격 | 일정 방식에 따라... | 10 4 * * * |
    | 시작시 일정 등록 | 선택 | On |
...
- **Plexmate Ready 새로고침**
    ...
    | 항목 | 값 | 예시 |
    | --- | --- | --- |
    | 설명 | 선택 | plex_mate READY 항목을 새로고침 |
    | VFS 리모트 | **필수** | gds: |
    | recursive | 선택 | Off |
    | 일정 방식 | 선택 | 시간 간격 |
    | 시간 간격 | 일정 방식에 따라... | */10 * * * * |
...
- **Plexmate 파일 정리**
    ...
    | 항목 | 값 | 예시 |
    | --- | --- | --- |
    | 설명 | 선택 | 최신 영화 섹션 파일 정리 |
    | 파일 정리 유형 | **필수** | 영화 |
    | 파일 정리 단계 | **필수** | 2단계 |
    | 파일 정리 섹션 | **필수** | 최신 영화 |
    | 일정 방식 | 선택 | 시간 간격 |
    | 시간 간격 | 일정 방식에 따라... | 10 6 * * * |
    | 시작시 일정 등록 | 선택 | On |
